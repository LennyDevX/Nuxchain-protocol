...
contract ReferralSystem is AccessControl, Initializable, UUPSUpgradeable {
        ...
        returns (uint256 discountAmount) 
    {
        ...
        if (levelingSystemAddress != address(0)) {
            try IXPHub(levelingSystemAddress).awardXP(buyer, BUYER_FIRST_PURCHASE_XP, IXPHub.XPSource.REFERRAL) {} catch {}
        }
        ...
        returns (bool) 
    {
        ...
        // Otorgar XP a referrer
        if (levelingSystemAddress != address(0)) {
            try IXPHub(levelingSystemAddress).awardXP(referrerAddr, REFERRER_FIRST_SALE_XP, IXPHub.XPSource.REFERRAL) {} catch {}
        }
