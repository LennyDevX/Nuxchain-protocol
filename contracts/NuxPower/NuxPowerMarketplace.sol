...
import "../interfaces/INuxPower.sol";
...
/**
 * @title NuxPowerMarketplace
 * @dev Marketplace for purchasing and managing NuxPowers without NFT minting
 * 
 * KEY FEATURES:
 ...
 * - Integrates with SmartStaking for reward multipliers
 * 
 ...
 */
contract NuxPowerMarketplace is AccessControl, Pausable, ReentrancyGuard, INuxPower {
    ...
    // Core storage
    mapping(uint256 => NuxPower) public nuxPowers;                              // skillId => NuxPower
    mapping(address => uint256[]) public usernuxPowers;                                 // user => [skillIds]
    ...
    mapping(IStakingIntegration.PowerType => mapping(IStakingIntegration.Rarity => uint16)) public skillEffectValues;
    ...
    mapping(IStakingIntegration.PowerType => uint8) public skillLevelCount;
    
    // Old pricing mapping (kept for backward compatibility - will be overridden by new system)
    mapping(IStakingIntegration.PowerType => mapping(IStakingIntegration.Rarity => uint256)) public skillPrices;
    
    // Dashboard statistics tracking
    uint256 public totalSkillsSold;
    uint256 public totalRevenue;
    mapping(IStakingIntegration.PowerType => uint256) public skillsSoldByType;
    mapping(IStakingIntegration.Rarity => uint256) public skillsSoldByRarity;
    mapping(IStakingIntegration.PowerType => uint256) public revenueBySkillType;
    ...
    error InvalidPowerType(uint8 skillType);
    ...
    // ════════════════════════════════════════════════════════════════════════════════════════
    // EVENTS (Additional to INuxPower)
    // ════════════════════════════════════════════════════════════════════════════════════════
    ...
    event PowerCleanedUp(address indexed user, uint256 indexed skillId);
    event SkillPricingUpdated(IStakingIntegration.PowerType indexed skillType, uint256 basePrice, uint256 rarityMultiplier);
    event PowerPurchaseProcessed(address indexed user, uint256 indexed skillId, IStakingIntegration.PowerType skillType, uint256 amount, string operation);
    event EmergencyWithdrawal(address indexed admin, uint256 amount, address indexed to);
    event PowerEffectValueUpdated(IStakingIntegration.PowerType indexed skillType, IStakingIntegration.Rarity indexed rarity, uint16 newEffectValue);
    event PowerLevelMultiplierUpdated(uint8 indexed level, uint16 newMultiplier);
    ...
    function _initializeSkillEffects() internal {
        ...
        skillEffectValues[IStakingIntegration.PowerType.STAKE_BOOST_I][IStakingIntegration.Rarity.COMMON] = 500;      // +5% APY
        skillEffectValues[IStakingIntegration.PowerType.STAKE_BOOST_I][IStakingIntegration.Rarity.UNCOMMON] = 700;    // +7% APY
        skillEffectValues[IStakingIntegration.PowerType.STAKE_BOOST_I][IStakingIntegration.Rarity.RARE] = 900;        // +9% APY
        skillEffectValues[IStakingIntegration.PowerType.STAKE_BOOST_I][IStakingIntegration.Rarity.EPIC] = 1200;       // +12% APY
        skillEffectValues[IStakingIntegration.PowerType.STAKE_BOOST_I][IStakingIntegration.Rarity.LEGENDARY] = 1600;  // +16% APY (less aggressive)
        
        // STAKE_BOOST_II (Skill #2): Base +10% APY (1000 bps) - COSTS 50% MORE
        skillEffectValues[IStakingIntegration.PowerType.STAKE_BOOST_II][IStakingIntegration.Rarity.COMMON] = 1000;    // +10% APY
        skillEffectValues[IStakingIntegration.PowerType.STAKE_BOOST_II][IStakingIntegration.Rarity.UNCOMMON] = 1300;  // +13% APY
        skillEffectValues[IStakingIntegration.PowerType.STAKE_BOOST_II][IStakingIntegration.Rarity.RARE] = 1600;      // +16% APY
        skillEffectValues[IStakingIntegration.PowerType.STAKE_BOOST_II][IStakingIntegration.Rarity.EPIC] = 2100;      // +21% APY
        skillEffectValues[IStakingIntegration.PowerType.STAKE_BOOST_II][IStakingIntegration.Rarity.LEGENDARY] = 2800; // +28% APY (was 50%, now 28%)
        
        // STAKE_BOOST_III (Skill #3): Base +15% APY (1500 bps) - COSTS 120% MORE THAN I
        skillEffectValues[IStakingIntegration.PowerType.STAKE_BOOST_III][IStakingIntegration.Rarity.COMMON] = 1500;    // +15% APY
        skillEffectValues[IStakingIntegration.PowerType.STAKE_BOOST_III][IStakingIntegration.Rarity.UNCOMMON] = 1900;  // +19% APY
        skillEffectValues[IStakingIntegration.PowerType.STAKE_BOOST_III][IStakingIntegration.Rarity.RARE] = 2300;      // +23% APY
        skillEffectValues[IStakingIntegration.PowerType.STAKE_BOOST_III][IStakingIntegration.Rarity.EPIC] = 3000;      // +30% APY
        skillEffectValues[IStakingIntegration.PowerType.STAKE_BOOST_III][IStakingIntegration.Rarity.LEGENDARY] = 3800; // +38% APY (was 90%, now 38%)
        
        // AUTO_COMPOUND (Skill #4): Automatic compounding
        skillEffectValues[IStakingIntegration.PowerType.AUTO_COMPOUND][IStakingIntegration.Rarity.COMMON] = 1000;      // Base 1x
        skillEffectValues[IStakingIntegration.PowerType.AUTO_COMPOUND][IStakingIntegration.Rarity.UNCOMMON] = 1300;    // 1.3x
        skillEffectValues[IStakingIntegration.PowerType.AUTO_COMPOUND][IStakingIntegration.Rarity.RARE] = 1600;        // 1.6x
        skillEffectValues[IStakingIntegration.PowerType.AUTO_COMPOUND][IStakingIntegration.Rarity.EPIC] = 2100;        // 2.1x
        skillEffectValues[IStakingIntegration.PowerType.AUTO_COMPOUND][IStakingIntegration.Rarity.LEGENDARY] = 2600;   // 2.6x (was 4.0x)
        
        // LOCK_REDUCER (Skill #5): Reduce lock time
        skillEffectValues[IStakingIntegration.PowerType.LOCK_REDUCER][IStakingIntegration.Rarity.COMMON] = 1500;       // -15% lock time
        skillEffectValues[IStakingIntegration.PowerType.LOCK_REDUCER][IStakingIntegration.Rarity.UNCOMMON] = 2000;     // -20% lock time
        skillEffectValues[IStakingIntegration.PowerType.LOCK_REDUCER][IStakingIntegration.Rarity.RARE] = 2500;         // -25% lock time
        skillEffectValues[IStakingIntegration.PowerType.LOCK_REDUCER][IStakingIntegration.Rarity.EPIC] = 3500;         // -35% lock time
        skillEffectValues[IStakingIntegration.PowerType.LOCK_REDUCER][IStakingIntegration.Rarity.LEGENDARY] = 4500;   // -45% lock time (was -90%)
        
        // FEE_REDUCER_I (Skill #6): -10% platform fees base
        skillEffectValues[IStakingIntegration.PowerType.FEE_REDUCER_I][IStakingIntegration.Rarity.COMMON] = 1000;      // -10% fees
        skillEffectValues[IStakingIntegration.PowerType.FEE_REDUCER_I][IStakingIntegration.Rarity.UNCOMMON] = 1300;    // -13% fees
        skillEffectValues[IStakingIntegration.PowerType.FEE_REDUCER_I][IStakingIntegration.Rarity.RARE] = 1600;        // -16% fees
        skillEffectValues[IStakingIntegration.PowerType.FEE_REDUCER_I][IStakingIntegration.Rarity.EPIC] = 2200;        // -22% fees
        skillEffectValues[IStakingIntegration.PowerType.FEE_REDUCER_I][IStakingIntegration.Rarity.LEGENDARY] = 2800;   // -28% fees (was -50%)
        
        // FEE_REDUCER_II (Skill #7): -20% platform fees base - COSTS 43% MORE THAN I
        skillEffectValues[IStakingIntegration.PowerType.FEE_REDUCER_II][IStakingIntegration.Rarity.COMMON] = 1800;     // -18% fees
        skillEffectValues[IStakingIntegration.PowerType.FEE_REDUCER_II][IStakingIntegration.Rarity.UNCOMMON] = 2300;   // -23% fees
        skillEffectValues[IStakingIntegration.PowerType.FEE_REDUCER_II][IStakingIntegration.Rarity.RARE] = 2800;       // -28% fees
        skillEffectValues[IStakingIntegration.PowerType.FEE_REDUCER_II][IStakingIntegration.Rarity.EPIC] = 3700;       // -37% fees
        skillEffectValues[IStakingIntegration.PowerType.FEE_REDUCER_II][IStakingIntegration.Rarity.LEGENDARY] = 4500; // -45% fees (was -90%)
        
        // ═══ ACTIVE SKILLS (8-16) ═══
        
        // PRIORITY_LISTING (Skill #8)
        skillEffectValues[IStakingIntegration.PowerType.PRIORITY_LISTING][IStakingIntegration.Rarity.COMMON] = 1000;
        skillEffectValues[IStakingIntegration.PowerType.PRIORITY_LISTING][IStakingIntegration.Rarity.UNCOMMON] = 1500;
        skillEffectValues[IStakingIntegration.PowerType.PRIORITY_LISTING][IStakingIntegration.Rarity.RARE] = 2000;
        skillEffectValues[IStakingIntegration.PowerType.PRIORITY_LISTING][IStakingIntegration.Rarity.EPIC] = 3000;
        skillEffectValues[IStakingIntegration.PowerType.PRIORITY_LISTING][IStakingIntegration.Rarity.LEGENDARY] = 5000;
        
        // BATCH_MINTER (Skill #9)
        skillEffectValues[IStakingIntegration.PowerType.BATCH_MINTER][IStakingIntegration.Rarity.COMMON] = 1000;
        skillEffectValues[IStakingIntegration.PowerType.BATCH_MINTER][IStakingIntegration.Rarity.UNCOMMON] = 1500;
        skillEffectValues[IStakingIntegration.PowerType.BATCH_MINTER][IStakingIntegration.Rarity.RARE] = 2000;
        skillEffectValues[IStakingIntegration.PowerType.BATCH_MINTER][IStakingIntegration.Rarity.EPIC] = 3000;
        skillEffectValues[IStakingIntegration.PowerType.BATCH_MINTER][IStakingIntegration.Rarity.LEGENDARY] = 5000;
        
        // VERIFIED_CREATOR (Skill #10)
        skillEffectValues[IStakingIntegration.PowerType.VERIFIED_CREATOR][IStakingIntegration.Rarity.COMMON] = 1000;
        skillEffectValues[IStakingIntegration.PowerType.VERIFIED_CREATOR][IStakingIntegration.Rarity.UNCOMMON] = 1500;
        skillEffectValues[IStakingIntegration.PowerType.VERIFIED_CREATOR][IStakingIntegration.Rarity.RARE] = 2000;
        skillEffectValues[IStakingIntegration.PowerType.VERIFIED_CREATOR][IStakingIntegration.Rarity.EPIC] = 3000;
        skillEffectValues[IStakingIntegration.PowerType.VERIFIED_CREATOR][IStakingIntegration.Rarity.LEGENDARY] = 5000;
        
        // INFLUENCER (Skill #11) - Less aggressive legendary
        skillEffectValues[IStakingIntegration.PowerType.INFLUENCER][IStakingIntegration.Rarity.COMMON] = 10000;  // 2x weight
        skillEffectValues[IStakingIntegration.PowerType.INFLUENCER][IStakingIntegration.Rarity.UNCOMMON] = 12000; // 2.4x weight
        skillEffectValues[IStakingIntegration.PowerType.INFLUENCER][IStakingIntegration.Rarity.RARE] = 14000;     // 2.8x weight
        skillEffectValues[IStakingIntegration.PowerType.INFLUENCER][IStakingIntegration.Rarity.EPIC] = 18000;     // 3.6x weight
        skillEffectValues[IStakingIntegration.PowerType.INFLUENCER][IStakingIntegration.Rarity.LEGENDARY] = 24000; // 4.8x weight (was 10x)
        
        // CURATOR (Skill #12)
        skillEffectValues[IStakingIntegration.PowerType.CURATOR][IStakingIntegration.Rarity.COMMON] = 1000;
        skillEffectValues[IStakingIntegration.PowerType.CURATOR][IStakingIntegration.Rarity.UNCOMMON] = 1200;
        skillEffectValues[IStakingIntegration.PowerType.CURATOR][IStakingIntegration.Rarity.RARE] = 1400;
        skillEffectValues[IStakingIntegration.PowerType.CURATOR][IStakingIntegration.Rarity.EPIC] = 1800;
        skillEffectValues[IStakingIntegration.PowerType.CURATOR][IStakingIntegration.Rarity.LEGENDARY] = 2400;
        
        // AMBASSADOR (Skill #13): referral bonus base - Matches GameifiedMarketplaceCoreV1 multiplier tiers
        skillEffectValues[IStakingIntegration.PowerType.AMBASSADOR][IStakingIntegration.Rarity.COMMON] = 15000;  // 1.5x
        skillEffectValues[IStakingIntegration.PowerType.AMBASSADOR][IStakingIntegration.Rarity.UNCOMMON] = 20000; // 2.0x
        skillEffectValues[IStakingIntegration.PowerType.AMBASSADOR][IStakingIntegration.Rarity.RARE] = 26000;     // 2.6x
        skillEffectValues[IStakingIntegration.PowerType.AMBASSADOR][IStakingIntegration.Rarity.EPIC] = 30000;     // 3.0x
        skillEffectValues[IStakingIntegration.PowerType.AMBASSADOR][IStakingIntegration.Rarity.LEGENDARY] = 35000; // 3.5x
        
        // VIP_ACCESS (Skill #14)
        skillEffectValues[IStakingIntegration.PowerType.VIP_ACCESS][IStakingIntegration.Rarity.COMMON] = 1000;
        skillEffectValues[IStakingIntegration.PowerType.VIP_ACCESS][IStakingIntegration.Rarity.UNCOMMON] = 1500;
        skillEffectValues[IStakingIntegration.PowerType.VIP_ACCESS][IStakingIntegration.Rarity.RARE] = 2000;
        skillEffectValues[IStakingIntegration.PowerType.VIP_ACCESS][IStakingIntegration.Rarity.EPIC] = 3000;
        skillEffectValues[IStakingIntegration.PowerType.VIP_ACCESS][IStakingIntegration.Rarity.LEGENDARY] = 5000;
        
        // EARLY_ACCESS (Skill #15): 24h early access
        skillEffectValues[IStakingIntegration.PowerType.EARLY_ACCESS][IStakingIntegration.Rarity.COMMON] = 1000;
        skillEffectValues[IStakingIntegration.PowerType.EARLY_ACCESS][IStakingIntegration.Rarity.UNCOMMON] = 1500;
        skillEffectValues[IStakingIntegration.PowerType.EARLY_ACCESS][IStakingIntegration.Rarity.RARE] = 2000;
        skillEffectValues[IStakingIntegration.PowerType.EARLY_ACCESS][IStakingIntegration.Rarity.EPIC] = 3000;
        skillEffectValues[IStakingIntegration.PowerType.EARLY_ACCESS][IStakingIntegration.Rarity.LEGENDARY] = 5000;
        
        // PRIVATE_AUCTIONS (Skill #16)
        skillEffectValues[IStakingIntegration.PowerType.PRIVATE_AUCTIONS][IStakingIntegration.Rarity.COMMON] = 1000;
        skillEffectValues[IStakingIntegration.PowerType.PRIVATE_AUCTIONS][IStakingIntegration.Rarity.UNCOMMON] = 1500;
        skillEffectValues[IStakingIntegration.PowerType.PRIVATE_AUCTIONS][IStakingIntegration.Rarity.RARE] = 2000;
        skillEffectValues[IStakingIntegration.PowerType.PRIVATE_AUCTIONS][IStakingIntegration.Rarity.EPIC] = 3000;
        skillEffectValues[IStakingIntegration.PowerType.PRIVATE_AUCTIONS][IStakingIntegration.Rarity.LEGENDARY] = 5000;
    }
    ...
    function purchaseNuxPower(
        ...
        if (uint8(_skillType) < MIN_SKILL_TYPE || uint8(_skillType) > MAX_SKILL_TYPE) {
            revert InvalidPowerType(uint8(_skillType));
        }
        ...
        // Store skill data
        NuxPower storage skill = nuxPowers[skillId];
        ...
        usernuxPowers[msg.sender].push(skillId);
        ...
        // Emit event
        emit nuxPowerPurchased(msg.sender, skillId, _skillType, _rarity, price);
        emit PowerPurchaseProcessed(msg.sender, skillId, _skillType, msg.value, "PURCHASE_INDIVIDUAL_SKILL");
     ...
     */
    function activateNuxPower(uint256 _skillId) external whenNotPaused {
        NuxPower storage skill = nuxPowers[_skillId];
        ...
        emit nuxPowerActivated(msg.sender, _skillId, skill.skillType);
    }
     ...
     */
    function deactivateNuxPower(uint256 _skillId) external whenNotPaused {
        NuxPower storage skill = nuxPowers[_skillId];
        ...
        emit nuxPowerDeactivated(msg.sender, _skillId, skill.skillType);
    }
     ...
     */
    function transferNuxPower(uint256 _skillId, address _recipient) external whenNotPaused {
        ...
        bool found = _removeFromArray(usernuxPowers[msg.sender], _skillId);
        if (!found) revert SkillNotInList(_skillId);
        
        // Transfer ownership
        skill.owner = _recipient;
        usernuxPowers[_recipient].push(_skillId);
        
        emit nuxPowerTransferred(msg.sender, _recipient, _skillId, skill.skillType);
    }
     ...
     */
    function renewNuxPower(uint256 _skillId) external payable whenNotPaused nonReentrant {
        NuxPower storage skill = nuxPowers[_skillId];
        ...
        emit nuxPowerRenewed(msg.sender, _skillId, skill.expiresAt);
        emit PowerPurchaseProcessed(msg.sender, _skillId, skill.skillType, msg.value, "RENEW_INDIVIDUAL_SKILL");
    ...
    function claimExpiredNuxPower(uint256 _skillId) external {
        ...
        bool ownershipRemoved = _removeFromArray(usernuxPowers[msg.sender], _skillId);
        if (!ownershipRemoved) revert SkillNotInList(_skillId);
        
        emit nuxPowerExpired(msg.sender, _skillId, skill.skillType);
        emit PowerCleanedUp(msg.sender, _skillId);
    }
    ...
    /// @notice Get a specific NuxPower by ID
    function getNuxPower(uint256 _skillId) external view returns (NuxPower memory) {
        return nuxPowers[_skillId];
    }
    
    /// @notice Get all NuxPowers owned by a user
    function getUsernuxPowers(address _user) external view returns (uint256[] memory) {
        return usernuxPowers[_user];
    }
    
    /// @notice Get active NuxPowers for a user by skill type
    function getUserActivenuxPowers(address _user, IStakingIntegration.PowerType /* _skillType */) external view returns (uint256[] memory) {
        return userActiveSkills[_user];
    }
    
    /// @notice Returns price for specific skill type and rarity
    function getnuxPowerPrice(IStakingIntegration.Rarity _rarity) external view returns (uint256) {
        // Returns price for ACTIVE skills COMMON rarity for backward compatibility
        return skillPrices[IStakingIntegration.PowerType(8)][_rarity];
    }
     ...
     */
    function getSkillPrice(IStakingIntegration.PowerType _skillType, IStakingIntegration.Rarity _rarity) external view returns (uint256) {
     ...
     */
    function getSkillPricesAllRarities(IStakingIntegration.PowerType _skillType) external view returns (uint256[5] memory prices) {
        for (uint8 i = 0; i < 5; i++) {
    ...
    function getAllSkillsPricing() external view returns (
        IStakingIntegration.PowerType[] memory skillTypes,
        uint8[] memory categories,
        uint256[][] memory prices
    ) {
        skillTypes = new IStakingIntegration.PowerType[](16);
    ...
    /// @notice Get detailed information about user's NuxPowers
    function getUsernuxPowersDetailed(address _user) external view returns (NuxPower[] memory skills, bool[] memory isActive) {
        uint256[] memory skillIds = usernuxPowers[_user];
        skills = new NuxPower[](skillIds.length);
        isActive = new bool[](skillIds.length);
        
        for (uint256 i = 0; i < skillIds.length; i++) {
            skills[i] = nuxPowers[skillIds[i]];
     ...
     */
    function getActivenuxPowers(address _user) external view returns (uint256[] memory activeSkillIds) {
        uint256[] memory allSkills = usernuxPowers[_user];
        uint256 activeCount = 0;
        
        // Count active
        for (uint256 i = 0; i < allSkills.length; i++) {
            if (block.timestamp < nuxPowers[allSkills[i]].expiresAt) {
                activeCount++;
            }
        }
    ...
    /**
     * @dev Get ALL user's NuxPowers grouped by category (STAKING vs ACTIVE)
     * @param _user Address of user
    ...
    function getUserSkillsByCategory(address _user) external view returns (
        NuxPower[] memory stakingSkills,
        NuxPower[] memory activeSkills
    ) {
        uint256[] memory allSkillIds = usernuxPowers[_user];
        ...
        for (uint256 i = 0; i < allSkillIds.length; i++) {
            NuxPower memory skill = nuxPowers[allSkillIds[i]];
        ...
        stakingSkills = new NuxPower[](stakingCount);
        activeSkills = new NuxPower[](activeCount);
    ...
    function getUserStakingSkills(address _user) external view returns (
        ...
        uint256 stakingCount = 0;
        for (uint256 i = 0; i < allSkillIds.length; i++) {
            uint8 skillTypeValue = uint8(nuxPowers[allSkillIds[i]].skillType);
            if (skillTypeValue >= 1 && skillTypeValue <= 7) {
                stakingCount++;
            }
        }
        ...
        // Initialize arrays
        skills = new NuxPower[](stakingCount);
        ...
        for (uint256 i = 0; i < allSkillIds.length; i++) {
            ...
            NuxPower memory skill = nuxPowers[skillId];
            uint8 skillTypeValue = uint8(skill.skillType);
    ...
    function getUserPlatformSkills(address _user) external view returns (
        ...
        // Initialize arrays
        skills = new NuxPower[](platformCount);
    ...
    function getUserSkillStats(address _user) external view returns (
        ...
        for (uint256 i = 0; i < totalSkills; i++) {
            NuxPower memory skill = nuxPowers[skillIds[i]];
            uint8 skillTypeValue = uint8(skill.skillType);
     ...
     */
    function _calculateSkillPrice(IStakingIntegration.PowerType _skillType, IStakingIntegration.Rarity _rarity) internal view returns (uint256) {
     ...
     */
    function _calculateSkillEffect(IStakingIntegration.PowerType _skillType, IStakingIntegration.Rarity _rarity) internal view returns (uint16) {
     ...
     */
    function _getSkillTypeMultiplier(IStakingIntegration.PowerType _skillType) internal pure returns (uint16) {
        // Staking Skills with different multipliers
        if (_skillType == IStakingIntegration.PowerType.STAKE_BOOST_I) {
            return 10000; // 1.0x base
        } else if (_skillType == IStakingIntegration.PowerType.STAKE_BOOST_II) {
            return 15000; // 1.5x (50% more expensive than Boost I)
        } else if (_skillType == IStakingIntegration.PowerType.STAKE_BOOST_III) {
            return 22000; // 2.2x (120% more expensive than Boost I)
        } else if (_skillType == IStakingIntegration.PowerType.AUTO_COMPOUND) {
            return 18000; // 1.8x 
        } else if (_skillType == IStakingIntegration.PowerType.LOCK_REDUCER) {
            return 16000; // 1.6x
        } else if (_skillType == IStakingIntegration.PowerType.FEE_REDUCER_I) {
            return 14000; // 1.4x
        } else if (_skillType == IStakingIntegration.PowerType.FEE_REDUCER_II) {
    ...
    function setLevelMultiplier(uint8 _levelIndex, uint16 _newMultiplier) external onlyRole(ADMIN_ROLE) {
        ...
        emit PowerLevelMultiplierUpdated(_levelIndex, _newMultiplier);
    }
    ...
    function setSkillEffectValue(
        ...
        emit PowerEffectValueUpdated(_skillType, _rarity, _effectValue);
    }
    ...
    function setSkillEffectValuesAllRarities(
        ...
        emit PowerEffectValueUpdated(_skillType, IStakingIntegration.Rarity.COMMON, _commonEffect);
        emit PowerEffectValueUpdated(_skillType, IStakingIntegration.Rarity.UNCOMMON, _uncommonEffect);
        emit PowerEffectValueUpdated(_skillType, IStakingIntegration.Rarity.RARE, _rareEffect);
        emit PowerEffectValueUpdated(_skillType, IStakingIntegration.Rarity.EPIC, _epicEffect);
        emit PowerEffectValueUpdated(_skillType, IStakingIntegration.Rarity.LEGENDARY, _legendaryEffect);
    }
     ...
     */
    function getSkillEffectValuesAllRarities(IStakingIntegration.PowerType _skillType) external view returns (uint16[5] memory effectValues) {
        if (uint8(_skillType) < MIN_SKILL_TYPE || uint8(_skillType) > MAX_SKILL_TYPE) {
            revert InvalidPowerType(uint8(_skillType));
        }
    ...
    /**
     * @dev Get NuxPowers marketplace statistics
     */
    function getnuxPowersMarketStats() external view returns (
        uint256 skillsSold,
        ...
        for (uint256 i = 0; i < counter; i++) {
            if (nuxPowers[i].owner != address(0) && nuxPowers[i].isActive) {
                activeCount++;
            }
        }
        ...
        for (uint256 i = 0; i < counter; i++) {
            address owner = nuxPowers[i].owner;
            if (owner != address(0) && usernuxPowers[owner].length > 0) {
                buyerCount++;
                break;
            }
        }
    ...
    function getSkillRevenueByType() external view returns (
        ...
        skillTypes = new IStakingIntegration.PowerType[](totalTypes);
    ...
    function getUserSkillSpending(address _user) external view returns (
        ...
        IStakingIntegration.PowerType mostPurchasedType
    ) {
        totalSpent = userTotalSpent[_user];
        skillsPurchased = usernuxPowers[_user].length;
        ...
        uint256[] memory userSkills = usernuxPowers[_user];
        if (userSkills.length > 0) {
            mostPurchasedType = nuxPowers[userSkills[0]].skillType;
        } else {
            mostPurchasedType = IStakingIntegration.PowerType.NONE;
        }
    }
