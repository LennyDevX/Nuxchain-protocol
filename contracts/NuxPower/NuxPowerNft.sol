 ...
 * NuxPowers are now managed by NuxPowerMarketplace.sol
 * 
...
contract NuxPowerNft is AccessControl, Pausable, ReentrancyGuard {
    ...
    IStakingIntegration.PowerType private constant DEFAULT_FREE_POWER_TYPE = IStakingIntegration.PowerType.STAKE_BOOST_I;
    IStakingIntegration.Rarity private constant DEFAULT_FREE_POWER_RARITY = IStakingIntegration.Rarity.COMMON;
    uint256 private constant DEFAULT_FREE_POWER_LEVEL = 1;
    ...
    mapping(IStakingIntegration.PowerType => uint256) public powerTypeCount;
    mapping(uint256 => bool) public isFirstSkillFree;
    mapping(address => bool) public hasClaimedFreeSkill;
    mapping(address => mapping(IStakingIntegration.PowerType => uint256)) public userActivePowersByType;
    mapping(address => mapping(IStakingIntegration.PowerType => uint256)) public userPowerNFTId;
    ...
    event PowerAdded(uint256 indexed tokenId, IStakingIntegration.PowerType skillType, IStakingIntegration.Rarity rarity);
    event PowerExpired(address indexed user, uint256 indexed tokenId, IStakingIntegration.PowerType skillType);
    event PowerRenewed(address indexed user, uint256 indexed tokenId, uint256 newExpiryTime);
    event PowerTypeAlreadyActive(address indexed user, IStakingIntegration.PowerType skillType);
    event SkillSwitched(
    ...
    event PowerDeactivatedManually(
        address indexed user,
        uint256 indexed tokenId,
        IStakingIntegration.PowerType skillType
    );
    ...
    event FreePowerClaimed(address indexed user, uint256 indexed tokenId, IStakingIntegration.PowerType skillType);
    ...
    error InvalidPowerType();
    error CoreContractNotSet();
    error PowerTypeAlreadyActiveDuplicate(IStakingIntegration.PowerType skillType);
    ...
    function _initializeSkillPricing() internal {
        ...
        for (uint8 skillType = 1; skillType <= 7; skillType++) {
            IStakingIntegration.PowerType st = IStakingIntegration.PowerType(skillType);
     ...
     */
    function _isBadgePower(IStakingIntegration.PowerType _skillType) internal pure returns (bool) {
        return (
            _skillType == IStakingIntegration.PowerType.MODERATOR ||
            _skillType == IStakingIntegration.PowerType.BETA_TESTER ||
            _skillType == IStakingIntegration.PowerType.VIP_PARTNER ||
            _skillType == IStakingIntegration.PowerType.AMBASSADOR
        );
    }
     ...
     */
    function _calculatePowerPrice(IStakingIntegration.PowerType _skillType, IStakingIntegration.Rarity _rarity) internal view returns (uint256) {
    ...
    function registerSkillsForNFT(
        uint256 _tokenId,
        IStakingIntegration.PowerType[] calldata _skillTypes,
        ...
        for (uint256 i = 0; i < _skillTypes.length; i++) {
            if (_isBadgePower(_skillTypes[i])) {
                hasBadgeSkill = true;
                break;
            }
        }
        ...
        if (!hasBadgeSkill && stakingContractAddress != address(0)) {
            try ISmartStaking(stakingContractAddress).getTotalDeposit(msg.sender) returns (uint256 totalDeposit) {
        ...
        for (uint256 i = 0; i < _skillTypes.length; i++) {
            if (includeFreeSkill && _skillTypes[i] == DEFAULT_FREE_POWER_TYPE) {
                revert DuplicatePowerType();
            }
            if (userActivePowersByType[msg.sender][_skillTypes[i]] != 0) {
                // This skill type is already active
                emit PowerTypeAlreadyActive(msg.sender, _skillTypes[i]);
                revert PowerTypeAlreadyActiveDuplicate(_skillTypes[i]);
            }
        }
        ...
        for (uint256 i = 0; i < _skillTypes.length; i++) {
            if (uint8(_skillTypes[i]) < 1 || uint8(_skillTypes[i]) > 19) revert InvalidPowerType();
            if (uint8(_rarities[i]) > 4) revert InvalidRarity(uint8(_rarities[i]));
            
            for (uint256 j = i + 1; j < _skillTypes.length; j++) {
                if (_skillTypes[i] == _skillTypes[j]) revert DuplicatePowerType();
            }
        }
        ...
        for (uint256 i = 0; i < _skillTypes.length; i++) {
            uint256 skillPrice = _calculatePowerPrice(_skillTypes[i], _rarities[i]);
        ...
        for (uint256 i = 0; i < _skillTypes.length; i++) {
            uint256 skillExpiry = _isBadgePower(_skillTypes[i]) ? type(uint256).max : block.timestamp + SKILL_DURATION;
            ...
            powerTypeCount[_skillTypes[i]]++;
            
            userActivePowersByType[msg.sender][_skillTypes[i]] = _tokenId;
            userPowerNFTId[msg.sender][_skillTypes[i]] = _tokenId;
            ...
            emit PowerAdded(_tokenId, _skillTypes[i], _rarities[i]);
        }

        if (includeFreeSkill) {
            Skill memory freeSkill = Skill(
                DEFAULT_FREE_POWER_TYPE,
                DEFAULT_FREE_POWER_RARITY,
                DEFAULT_FREE_POWER_LEVEL,
                block.timestamp,
            ...
            powerTypeCount[DEFAULT_FREE_POWER_TYPE]++;
            userActivePowersByType[msg.sender][DEFAULT_FREE_POWER_TYPE] = _tokenId;
            userPowerNFTId[msg.sender][DEFAULT_FREE_POWER_TYPE] = _tokenId;
            userTotalActiveSkills[msg.sender]++;
            totalXP += 15;
            hasClaimedFreeSkill[msg.sender] = true;
            emit PowerAdded(_tokenId, DEFAULT_FREE_POWER_TYPE, DEFAULT_FREE_POWER_RARITY);
            emit FreePowerClaimed(msg.sender, _tokenId, DEFAULT_FREE_POWER_TYPE);
        }
        ...
        IMarketplaceCore(coreContractAddress).updateUserXP(msg.sender, totalXP);
    ...
    function deactivateExpiredSkill(uint256 _tokenId) external {
        ...
        for (uint256 i = 0; i < nft.skills.length; i++) {
            IStakingIntegration.PowerType skillType = nft.skills[i].skillType;
            
            if (userActivePowersByType[nft.creator][skillType] == _tokenId) {
                userActivePowersByType[nft.creator][skillType] = 0;
                userPowerNFTId[nft.creator][skillType] = 0;
            ...
            emit PowerExpired(nft.creator, _tokenId, skillType);
        }
    }
    ...
    function renewSkill(uint256 _tokenId) external payable whenNotPaused nonReentrant {
        ...
        emit PowerRenewed(msg.sender, _tokenId, newExpiryTime);
    }
    ...
    function switchSkill(
        ...
        if (!oldHasSkill) revert InvalidPowerType();
        if (userActivePowersByType[msg.sender][_skillType] != _oldTokenId) revert InvalidPowerType();
        ...
        if (!newHasSkill) revert InvalidPowerType();
        ...
        userActivePowersByType[msg.sender][_skillType] = _newTokenId;
        userPowerNFTId[msg.sender][_skillType] = _newTokenId;
     ...
     */
    function deactivatePowerManually(
        uint256 _tokenId,
        IStakingIntegration.PowerType _skillType
    ) external whenNotPaused {
        SkillNFT storage nft = skillNFTs[_tokenId];
        if (nft.creator != msg.sender) revert NotSkillOwner();
        if (userActivePowersByType[msg.sender][_skillType] != _tokenId) revert InvalidPowerType();
        ...
        if (!found) revert InvalidPowerType();
        ...
        userActivePowersByType[msg.sender][_skillType] = 0;
        userPowerNFTId[msg.sender][_skillType] = 0;
        ...
        emit PowerDeactivatedManually(msg.sender, _tokenId, _skillType);
    }
    ...
    function getpowerTypeCount(IStakingIntegration.PowerType _skillType) external view returns (uint256) {
        return powerTypeCount[_skillType];
    }
    ...
    function getPowerSwitchInfo(
        ...
        activeTokenId = userActivePowersByType[_user][_skillType];
    ...
    function getUserPowersByType(
        ...
        uint256 index = 0;
        uint256 activeToken = userActivePowersByType[_user][_skillType];
    ...
    /**
     * @dev Update NuxPower price (admin only)
     */
    function updateSkillPrice(IStakingIntegration.PowerType _skillType, IStakingIntegration.Rarity _rarity, uint256 _newPrice) external onlyRole(ADMIN_ROLE) {
        if (uint8(_skillType) < 1 || uint8(_skillType) > 16) revert InvalidPowerType();
     ...
     */
    function getSkillPricesAllRarities(IStakingIntegration.PowerType _skillType) external view returns (uint256[5] memory) {
    ...
    function getPopularSkills(uint256 _limit) external view returns (
        ...
        for (uint8 i = 1; i <= totalSkillTypes; i++) {
            counts[i - 1] = powerTypeCount[IStakingIntegration.PowerType(i)];
        }
        
        uint256 resultSize = _limit < totalSkillTypes ? _limit : totalSkillTypes;
        skillTypes = new IStakingIntegration.PowerType[](resultSize);
        ...
        for (uint256 i = 0; i < resultSize; i++) {
            ...
            skillTypes[i] = IStakingIntegration.PowerType(maxIndex + 1);
    ...
    function getUserSkillsGrouped(address _user) external view returns (
        ...
        IStakingIntegration.PowerType[] memory tempTypes = new IStakingIntegration.PowerType[](maxSize);
        ...
        skillTypes = new IStakingIntegration.PowerType[](uniqueCount);
