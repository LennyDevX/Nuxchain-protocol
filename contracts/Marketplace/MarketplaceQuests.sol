...
contract MarketplaceQuests is AccessControl, Pausable, ReentrancyGuard {
    ...
    function completeQuest(uint256 _questId) external whenNotPaused nonReentrant {
        ...
        if (levelingContractAddress != address(0)) {
            try IXPHub(levelingContractAddress).awardXP(msg.sender, quest.xpReward, IXPHub.XPSource.QUEST) {} catch {}
        }
    ...
    function _calculateQuestProgress(address _user, Quest memory _quest) internal view returns (uint256) {
        ...
        IXPHub.UserProfile memory profile = IXPHub(levelingContractAddress).getUserProfile(_user);
